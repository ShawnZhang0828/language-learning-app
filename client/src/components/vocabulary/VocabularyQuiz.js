...
import {
  getFeedback,
  getVocabularyQuizQuestions,
} from "../../controllers/VocabularyQuizController";
...
function VocabularyQuiz() {
  ...
  const onQuizStartClick = async (diff, numberOfQuestions) => {
    var allQuestions = await getVocabularyQuizQuestions(
      diff,
      numberOfQuestions
    );
  ...
  const onQuizSubmitClick = async () => {
    const firstFeedbackResponse = await getFeedback(
      firstSetAnswer,
      userPreference["target-language"],
      userPreference["original-language"]
    );
    const secondFeedbackResponse = await getFeedback(
      secondSetAnswer,
      userPreference["original-language"],
      userPreference["target-language"]
    );
  ...
  return (
    <div id="vocabulary-quiz-page">
      ...
      <div
        ...
        style={{
          display: `${
            firstQuestionSet.length === 0 || secondQuestionSet.length === 0
              ? "none"
              : ""
          }`,
        }}
